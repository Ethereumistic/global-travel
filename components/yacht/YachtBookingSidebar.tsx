              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel>Период на наемане</FormLabel>
                      ...
                      <PopoverTrigger asChild>
                        <FormControl>
                          <Button 
                            ...
                            variant={"outline"} 
                            className={cn(
                              "w-full pl-3 text-left font-normal h-12 border-slate-200 hover:bg-slate-50", 
                            ...
                            {field.value?.from ? (
                              field.value.to ? (
                                <div className="flex items-center gap-2 w-full">
                                    <span className="font-semibold text-slate-700">{format(field.value.from, "dd MMM", { locale: bg })}</span>
                                    <span className="text-slate-400">→</span>
                                    <span className="font-semibold text-slate-700">{format(field.value.to, "dd MMM", { locale: bg })}</span>
                                    <span className="ml-auto text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded-full font-medium">{differenceInDays(field.value.to, field.value.from)} дни</span>
                                </div>
                              ) : (format(field.value.from, "dd MMM yyyy", { locale: bg }))
                            ) : (<span className="flex items-center gap-2"><CalendarIcon className="h-4 w-4 opacity-50" />Изберете дати</span>)}
                          </Button>
                        </FormControl>
                      </PopoverTrigger>
                ...
                render={({ field }) => (
                  <FormItem>
                     <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-xs text-muted-foreground bg-slate-100 px-2 py-0.5 rounded">Макс: {yacht.guests} гости</span>
                     </div>
                     <FormControl>
                        <div className="flex items-center gap-3">
                           ...
                           <Button type="button" variant="outline" size="icon" className="h-11 w-11 shrink-0 border-slate-200" onClick={() => handleGuestChange(1)} disabled={field.value >= (parseInt(yacht.guests) || 12)}><Plus className="h-4 w-4" /></Button>
                        </div>
                     </FormControl>
                     <FormMessage />
                  </FormItem>
